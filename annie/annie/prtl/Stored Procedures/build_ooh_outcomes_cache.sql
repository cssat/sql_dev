CREATE PROCEDURE [prtl].[build_ooh_outcomes_cache]
...
WHERE NOT EXISTS (
		SELECT *
		FROM prtl.ooh_outcomes_cache_query cq
...
BEGIN
	...
	SELECT
		...
		,ooh.discharge_count
		,ooh.cohort_count
		,CONVERT(DECIMAL(9, 2), ooh.discharge_count / (ooh.cohort_count * 1.00) * 100) [rate]
	FROM (
		SELECT
			ooh.qry_type
			,ooh.date_type
			...
			,ooh.month
			,ooh.discharge_count
			,SUM(ooh.cohort_count) OVER (PARTITION BY ooh.qry_type
				,ooh.date_type
				,ooh.cohort_entry_date
				,ooh.age_grouping_cd
				,ooh.pk_gender
		...
		FROM (
			SELECT ooh.qry_type
				...
				,ooh.cd_discharge_type
				,ooh.mnth [month]
				,ISNULL(SUM(ooh.discharge_count), 0) [discharge_count]
				,ISNULL(SUM(ooh.cohort_count), 0) [cohort_count]
			FROM @parameters p
			...
			INNER JOIN prtl.ooh_outcomes ooh ON ooh.age_grouping_cd = mag.age_grouping_match_code
				...
				AND ooh.Removal_County_Cd = mc.county_match_code

CREATE PROCEDURE [prtl].[sp_ooh_wb_family_settings] (
	@date VARCHAR(3000)
...
AS
EXEC prtl.log_query_ooh_wb_family_settings @age_grouping_cd
	,@pk_gender
	,@cd_race_census
...
EXEC prtl.build_ooh_wb_family_settings_cache @age_grouping_cd
	,@pk_gender
	,@cd_race_census
	,@initial_cd_placement_setting
	,@longest_cd_placement_setting
...
SELECT m.qry_type [qry_type_poc1_first_all]
	,m.cohort_entry_date [Date]
	,m.age_grouping_cd
	,m.pk_gender
	,m.cd_race_census
	...
	,m.cd_allegation
	,m.cd_finding
	,m.family_setting_dcfs_percentage [Family Setting (State Foster Home)]
	,m.family_setting_private_agency_percentage [Family Setting (Private Foster Home)]
	,m.relative_percentage [Family Setting (Kin Placement)]
	,m.group_inst_care_percentage [Non-Family Setting]
FROM prtl.ooh_wb_family_settings_cache m

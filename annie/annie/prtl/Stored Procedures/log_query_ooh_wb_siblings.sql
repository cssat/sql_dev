...
WHERE id = IIF(@is_pivot = 0, 13, 27); -- sp_ooh_wb_siblings, sp_ooh_wb_siblings_pvt
...
BEGIN
	...
	SELECT @age_grouping_cd
		...
		,@min_month_start
		,@max_month_start
		,IIF(@is_pivot = 0, 1, 0) [cnt_qry_siblings]
		,IIF(@is_pivot = 1, 1, 0) [cnt_qry_sibling_pvt]
		,GETDATE() [last_run_date];
END
ELSE
BEGIN
	UPDATE prtl.ooh_wb_siblings_params
	SET cnt_qry_siblings = IIF(@is_pivot = 0, cnt_qry_siblings + 1, cnt_qry_siblings)
		,cnt_qry_siblings_pvt = IIF(@is_pivot = 1, cnt_qry_siblings_pvt + 1, cnt_qry_siblings_pvt)

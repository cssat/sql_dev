CREATE PROCEDURE [prtl].[sp_ooh_pit_rates] (
	@date VARCHAR(3000)
...
AS
EXEC prtl.log_query_ooh_pit @age_grouping_cd
	,@pk_gender
	,@cd_race_census
...
EXEC prtl.build_ooh_pit_cache @age_grouping_cd
	,@pk_gender
	,@cd_race_census
	,@initial_cd_placement_setting
	,@longest_cd_placement_setting
...
SELECT m.qry_type [qry_type_first_all]
	,m.date_type
	,m.start_date [Date]
	,m.age_grouping_cd
	,m.pk_gender
	...
	,m.cd_allegation
	,m.cd_finding
	,m.rate_start_date [Total in Out-of-Home Care 1st Day]
FROM prtl.ooh_pit_cache m
...
INNER JOIN ref.vw_dependency_lag dl ON dl.bin_dependency_cd = m.bin_dependency_cd
	...
	AND m.start_date BETWEEN @min_filter_date AND dl.cohort_max_filter_date
INNER JOIN ref.filter_los los ON los.bin_los_cd = m.bin_los_cd
	AND IIF(m.bin_los_cd != 0, DATEADD(YEAR, 1, DATEADD(DAY, -1, DATEADD(DAY, ABS(los.lag_days), m.start_date))), @cutoff_date) <= @cutoff_date
INNER JOIN ref.filter_ihs_services ihs ON ihs.bin_ihs_service_cd = m.bin_ihs_service_cd
	AND ihs.min_display_date <= m.start_date
WHERE m.fl_include_perCapita = 1

...
AS
EXEC prtl.log_query_ooh_outcomes @age_grouping_cd
	,@pk_gender
	,@cd_race_census
...
EXEC prtl.build_ooh_outcomes_cache @age_grouping_cd
	,@pk_gender
	,@cd_race_census
	,@initial_cd_placement_setting
	,@longest_cd_placement_setting
...
SELECT m.month [Months Since Entering Out-of-Home Care]
	,m.cohort_entry_date [Cohort Period]
	...
	,m.cd_discharge_type
	,m.rate [Percent]
FROM prtl.ooh_outcomes_cache m
...
INNER JOIN ref.filter_los los ON los.bin_los_cd = m.bin_los_cd
	AND IIF(m.bin_los_cd != 0, DATEADD(YEAR, 1, DATEADD(MONTH, 9, DATEADD(DAY, -1, DATEADD(DAY, ABS(los.lag_days), m.cohort_entry_date)))), @cutoff_date) <= @cutoff_date
...
WHERE m.month = IIF(@months = 0, m.month, @months)
	AND DATEADD(MONTH, IIF(@months = 0, 12, 15) + ISNULL(m.month, 0), m.cohort_entry_date) <= @cutoff_date
ORDER BY m.cohort_entry_date ASC
	,m.qry_type

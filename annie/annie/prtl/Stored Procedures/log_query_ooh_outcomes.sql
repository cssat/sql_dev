CREATE PROCEDURE [prtl].[log_query_ooh_outcomes]
...
WHERE id = 23; -- sp_ooh_outcomes

SET @qry_id = (
		SELECT TOP 1 qry_id
		FROM prtl.ooh_outcomes_params
...
IF @qry_Id IS NULL
BEGIN
	INSERT INTO prtl.ooh_outcomes_params (
		age_grouping_cd
		,pk_gender
		,cd_race_census
	...
	SELECT @age_grouping_cd
		...
		,@min_month_start
		,@max_month_start
		,IIF(@months = 0, 1, 0) [cnt_qry_outcomes]
		,IIF(@months = 3, 1, 0) [cnt_qry_outcomes_3m]
		,IIF(@months = 12, 1, 0) [cnt_qry_outcomes_12m]
		,IIF(@months = 24, 1, 0) [cnt_qry_outcomes_24m]
...
ELSE
BEGIN
	UPDATE prtl.ooh_outcomes_params
	SET cnt_qry_outcomes = IIF(@months = 0, cnt_qry_outcomes + 1, cnt_qry_outcomes)
		,cnt_qry_outcomes_3m = IIF(@months = 3, cnt_qry_outcomes_3m + 1, cnt_qry_outcomes_3m)
		,cnt_qry_outcomes_12m = IIF(@months = 12, cnt_qry_outcomes_12m + 1, cnt_qry_outcomes_12m)
		,cnt_qry_outcomes_24m = IIF(@months = 24, cnt_qry_outcomes_24m + 1, cnt_qry_outcomes_24m)

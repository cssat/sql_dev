CREATE PROCEDURE [prtl].[build_ooh_reentry_cache]
...
WHERE NOT EXISTS (
		SELECT *
		FROM prtl.ooh_reentry_cache_query cq
...
BEGIN
	INSERT prtl.ooh_reentry_cache (
		...
		SELECT
			...
			,ooh.reentry_within_month
			,ooh.reentry_count
			,ooh.total_count
			,ROUND((ooh.reentry_count * 1.00) / (ooh.total_count * 1.00) * 100, 2) [reentry_rate]
		FROM (
			SELECT ooh.qry_type
				,ooh.date_type
				,ooh.cohort_exit_year
				...
				,ooh.reentry_within_month
				,ooh.reentry_count
				,SUM(ooh.total_count) OVER(PARTITION BY ooh.qry_type
					,ooh.date_type
					,ooh.cohort_exit_year
					,ooh.age_grouping_cd
					,ooh.pk_gender
			...
			FROM (
				SELECT ooh.qry_type
					...
					,p.cd_finding
					,ooh.cd_discharge_type
					,ooh.mnth [reentry_within_month]
					,SUM(ooh.discharge_count) [reentry_count]
					,SUM(ooh.cohort_count) [total_count]
				FROM @parameters p
				...
				INNER JOIN prtl.ooh_reentry ooh ON ooh.age_grouping_cd = mag.age_grouping_match_code
					AND ooh.pk_gndr = mg.pk_gender_match_code
					AND ooh.cd_race_census = mrc.race_census_match_code
					AND ooh.init_cd_plcm_setng = ips.placement_setting_match_code
					AND ooh.long_cd_plcm_setng = lps.placement_setting_match_code
					AND ooh.exit_county_cd = mc.county_match_code

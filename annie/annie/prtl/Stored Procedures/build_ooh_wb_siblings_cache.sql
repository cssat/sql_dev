CREATE PROCEDURE [prtl].[build_ooh_wb_siblings_cache]
...
WHERE NOT EXISTS (
		SELECT *
		FROM prtl.ooh_wb_siblings_cache_query cq
...
BEGIN
	...
	SELECT ooh.qry_type
		...
		,p.cd_finding
		,mk.kincare
		,msgs.bin_sibling_group_size
		,ROUND((SUM(ooh.all_sib_together) * 1.00) / (SUM(ooh.cnt_child) * 1.00) * 100, 2) [all_together]
		,ROUND((SUM(ooh.some_sib_together) * 1.00) / (SUM(ooh.cnt_child) * 1.00) * 100, 2) [some_together]
		,ROUND((SUM(ooh.no_sib_together) * 1.00) / (SUM(ooh.cnt_child) * 1.00) * 100, 2) [none_together]
		,SUM(ooh.cnt_child) [cnt_cohort]
	FROM @parameters p
	...
	INNER JOIN prtl.ooh_pit_measures ooh ON ooh.age_grouping_cd_mix = mag.age_grouping_match_code
		...
		AND ooh.fl_ooh_wb_siblings = 1
	INNER JOIN ref.match_kincare mk ON mk.kincare_match_code = ooh.kincare
	INNER JOIN ref.match_bin_sibling_group_size msgs ON msgs.sibling_group_size_match_code = ooh.bin_sibling_group_size

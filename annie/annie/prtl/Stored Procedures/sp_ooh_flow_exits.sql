...
AS
EXEC prtl.log_query_ooh_flow_exits @age_grouping_cd
	,@pk_gender
	,@cd_race_census
...
EXEC prtl.build_ooh_flow_exits_cache @age_grouping_cd
	,@pk_gender
	,@cd_race_census
	,@initial_cd_placement_setting
	,@longest_cd_placement_setting
...
SELECT m.qry_type
	...
	,m.bin_dependency_cd
	,m.cd_discharge_type
	,m.jit_exits [Number of Exits]
FROM prtl.ooh_flow_exits_cache m
...
INNER JOIN ref.vw_dependency_lag dl ON dl.bin_dependency_cd = m.bin_dependency_cd
	...
	AND m.start_date BETWEEN @min_filter_date AND dl.max_filter_date

CREATE PROCEDURE [prtl].[log_query_ooh_pit]
...
WHERE id = IIF(@is_counts = 1, 19, 20); -- sp_ooh_pit_counts, sp_ooh_pit_rates

SET @qry_id = (
		SELECT TOP 1 qry_id
		FROM prtl.ooh_pit_params
...
IF @qry_Id IS NULL
BEGIN
	INSERT INTO prtl.ooh_pit_params (
		age_grouping_cd
		,pk_gender
		,cd_race_census
	...
	SELECT @age_grouping_cd
		...
		,IIF(@is_counts = 0, 1, 0) [cnt_qry_rates]
		,GETDATE() [last_run_date];
END
ELSE
BEGIN
	UPDATE prtl.ooh_pit_params

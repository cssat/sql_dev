			...
			if @outcome_cd=1 
				begin
					...
					Select  
					  ...
					  join dbo.ref_lookup_hispanic_latino_census eth on eth.census_hispanic_latino_origin_cd=vw.ethnicity_cd
					  
					  --join dbo.ref_lookup_gender gdr on gdr.pk_gndr=0 -- > 0 and gdr.cd_gndr=tce.cd_gndr  

					  join ref_match_ooh_parameters prm on 
									  prm.age_grouping_cd=0--@age_grouping_cd--age.age_grouping_cd
								  and prm.pk_gndr=0 --gdr.pk_gndr
								  and prm.cd_race_census=eth.cd_race_census --tce.cd_race_census 

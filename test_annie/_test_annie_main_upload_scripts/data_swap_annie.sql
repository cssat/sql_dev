...
-- execute all four of these concat statements at the same time to generate the statements for the table moves and rollbacks
...
WHERE a.table_schema = @var_destination_schema and a.table_type = 'BASE TABLE' and (a.table_name like 'cache%' or a.table_name like 'ref_match%' or a.table_name like 'prtl%'
    ...
    and a.table_name != 'prtl_labels' and exists(select * from information_schema.tables r where r.table_name = a.table_name and r.table_schema = @var_source_schema and r.table_type = 'BASE TABLE')
...
WHERE r.table_schema = @var_source_schema and r.table_type = 'BASE TABLE' and (r.table_name like 'cache%' or r.table_name like 'ref_match%' or r.table_name like 'prtl%'
    ...
    and r.table_name != 'prtl_labels' and exists(select * from information_schema.tables a where a.table_name = r.table_name and a.table_schema = @var_destination_schema and a.table_type = 'BASE TABLE')

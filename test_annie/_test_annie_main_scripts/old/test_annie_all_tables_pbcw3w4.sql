...
fields terminated by '|'
(pit_date,date_type,qry_type,age_grouping_cd,pk_gndr,cd_race_census,census_hispanic_latino_origin_cd,init_cd_plcm_setng,long_cd_plcm_setng,county_cd,int_match_param_key,
bin_dep_cd,max_bin_los_cd,bin_placement_cd,cd_reporter_type,bin_ihs_svc_cd,filter_access_type,filter_allegation,filter_finding,filter_service_category,filter_service_budget,kincare,bin_sibling_group_size,all_sib_together,some_sib_together,no_sib_together,total);

analyze table prtl_pbcw4;

  update prtl_tables_last_update
  set load_date=now(),row_count=(select count(*) from prtl_pbcw4)
  where tbl_name='prtl_pbcw4';

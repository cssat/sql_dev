...
select 'cache_qry_param_pbcw3' as filename,count(*) from cache_qry_param_pbcw3;
...
select 'cache_pbcw3_params' as filename ,count(*) from cache_pbcw3_params;
...
bin_ihs_svc_cd, cd_reporter_type, cd_access_type, cd_allegation, cd_finding, cd_subctgry_poc_frc, cd_budget_poc_frc, age_grouping_cd, cd_race, pk_gndr, 
init_cd_plcm_setng, long_cd_plcm_setng, county_cd, family_setting_dcfs_prcntg, family_setting_private_agency_prcntg, relative_prcntg, group_inst_care_prcntg, 
min_start_date,  max_start_date, x1, x2, insert_date, qry_id, cohort_begin_year, int_hash_key);

analyze table cache_pbcw3_aggr;

select 'cache_pbcw3_aggr' as filename ,count(*) from cache_pbcw3_aggr;

...
SELECT rp.VICS_ID_CHILDREN [id_child_ic]
	,infa.ID_INTAKE_FACT [id_intake_ic]
	,rp.ID_ACCESS_REPORT [id_access_report]
INTO #ic
FROM base.rptIntake_children rp
LEFT JOIN dbo.INTAKE_FACT infa ON rp.ID_ACCESS_REPORT = infa.ID_ACCESS_REPORT
...
SELECT ic.id_child_ic [id_child_ic]
	,hc.id_prsn_child [id_child_hc]
	,ic.id_intake_ic [id_intake_ic]
	,hc.id_intake_fact [id_intake_hc]
INTO #raw_ic_hc
FROM base.tbl_household_children hc
FULL OUTER JOIN #ic ic ON hc.id_prsn_child = ic.id_child_ic
...
SELECT COALESCE(id_child_ic, id_child_hc) [id_child]
	,COALESCE(id_intake_ic, id_intake_hc) [id_intake]
	,ic_hc_selection
INTO #ic_hc
FROM #trans_ic_hc
...
SELECT ih.*
	,p.id_placement_fact [id_removal]
	,p.child [id_child_p]
INTO #raw_ic_hc_p
FROM #ic_hc ih
FULL OUTER JOIN base.rptPlacement p ON ih.id_child = p.child
...
-- record came from.
SELECT COALESCE(id_child, id_child_p) AS id_child
	,id_intake
	,id_removal
	,CASE 
...
-- a birth record.
SELECT bf.cd_birth_fact AS id_birth
	,cip.*
	,CASE 
	...
	,CASE 
		WHEN cip.ca_record_source IS NOT NULL
			...
			THEN 'yes'
		ELSE 'no'
		END [has_ca_and_birth_record]
INTO #raw_BCIR
FROM #ic_hc_p cip
FULL OUTER JOIN rodis_wh.birth_fact bf ON cip.id_child = bf.id_prsn_child
...
INTO #t1
FROM #raw_BCIR rb
LEFT JOIN dbo.PEOPLE_DIM pd ON rb.id_child = pd.ID_PRSN
	AND pd.IS_CURRENT = 1
LEFT JOIN rodis_wh.birth_fact bf ON rb.id_birth = bf.cd_birth_fact
...
SELECT t.*
	,CASE 
		...
		END [intake_date]
	,p_date.CALENDAR_DATE [removal_date]
INTO #t2
FROM #t1 t
LEFT JOIN INTAKE_FACT i ON t.id_intake = i.ID_INTAKE_FACT
LEFT JOIN dbo.PLACEMENT_FACT p ON t.id_removal = p.ID_PLACEMENT_FACT
LEFT JOIN dbo.CALENDAR_DIM i_date ON i.ID_CALENDAR_DIM_ACCESS_RCVD = i_date.ID_CALENDAR_DIM
LEFT JOIN dbo.CALENDAR_DIM p_date ON p.ID_CALENDAR_DIM_RMVL = p_date.ID_CALENDAR_DIM
...
SELECT *
	,CASE 
		...
		ELSE DENSE_RANK() OVER (
				PARTITION BY id_child
				,id_birth ORDER BY intake_date
					,id_intake
				)
		END [intake_order]
	,CASE 
...
AS (
	SELECT *
		...
		IIF(ISNULL(id_intake, 0) <> ISNULL(lag(id_intake, 1) OVER (
		...
		,
		-- Or a new removal.
		IIF(ISNULL(id_removal, 0) <> ISNULL(lag(id_removal, 1) OVER (
					PARTITION BY id_birth
					,id_child ORDER BY removal_order
						,intake_removal_order
					), 0), 1, 0) [new_removal]
	FROM #t3
	)
...
AS (
	SELECT *
		...
		SUM(new_intake) OVER (
			PARTITION BY id_birth
			,id_child ORDER BY intake_removal_order
			) [cumulative_intakes]
		,
...
AS (
	SELECT *
		...
		ROW_NUMBER() OVER (
			PARTITION BY id_birth
			,id_child
			,cumulative_intakes ORDER BY intake_removal_order
			) [last_intake_lag]
		,
		...
		ROW_NUMBER() OVER (
			PARTITION BY id_birth
			,id_child
			,cumulative_removals ORDER BY intake_removal_order
			) [bad_removal_lag]
	FROM temp2
	)
,temp4
...
SELECT *
	,FIRST_VALUE(intake_date) OVER (
		PARTITION BY id_birth
		,id_child ORDER BY intake_order
		) [first_intake_date]
...
-- focus.
SELECT *
	,DATEDIFF(DAY, child_birth_date, first_intake_date) [days_birth_to_first_intake]
	,DATEDIFF(DAY, child_birth_date, first_removal_date) [days_birth_to_first_removal]
	,DATEDIFF(DAY, child_birth_date, intake_date) [days_birth_to_intake]
	,DATEDIFF(DAY, child_birth_date, removal_date) [days_birth_to_removal]
	,DATEDIFF(DAY, first_intake_date, intake_date) [days_first_intake_to_intake]
	,DATEDIFF(DAY, previous_intake_date, intake_date) [days_previous_intake_to_intake]
	,DATEDIFF(DAY, intake_date, removal_date) [days_intake_to_removal]
	,DATEDIFF(DAY, previous_removal_date, removal_date) [days_previous_removal_to_removal]
INTO #t6
FROM #t5
...
SELECT t.*
	,bf.qt_child_weight [child_birth_weight_grams]
	,CASE 
		...
		WHEN bf.qt_child_weight < 2500
			THEN 'low'
		END [child_birth_weight_category]
	,CASE 
		WHEN t.has_birth_record = 'no'
			THEN NULL
...
LEFT JOIN rodis_wh.birth_administration_dim bad ON bf.id_birth_administration = bad.id_birth_administration
...
SELECT t.*
	,CASE 
		...
		WHEN bccd.fl_spinabifida NOT IN (9,0)
			...
			THEN 'yes'
		ELSE 'none recorded'
		END [any_congenital_abnormality]
	,CASE 
		WHEN t.has_birth_record = 'no'
			THEN NULL
		WHEN b.PARITY = 0
	...
	CASE 
		...
		WHEN bad.dt_prenatal_care_start >= 6
			...
			THEN 'third'
		END [prenatal_care_start_trimester]
	,
	...
	CASE 
		...
		END [birth_payment_category]
INTO #t8
FROM #t7 AS t
LEFT JOIN rodis_wh.birth_fact bf ON t.id_birth = bf.cd_birth_fact
LEFT JOIN rodis_wh.birth_administration_dim bad ON bf.id_birth_administration = bad.id_birth_administration
LEFT JOIN rodis_wh.birth_child_condition_dim bccd ON bf.birth_administration_id_birth_child_condition = bccd.id_birth_child_condition
LEFT JOIN rodis.berd b ON t.id_birth = b.bc_uni
LEFT JOIN rodis.ref_delivpay rd ON b.delivpay = rd.Code
...
SELECT t.*
	,CASE 
		...
		WHEN (
				b.dadbirst = 99
				OR b.dadbirst IS NULL
				)
			AND (
				b.dadrace = 99
				OR b.dadrace IS NULL
				)
			AND (
				b.dadhisp = 9
				OR b.dadhisp IS NULL
				)
			AND (
				b.dadocc = 999
				OR b.dadocc IS NULL
	...
	,CASE 
		...
		WHEN bf.qt_maternal_age >= 30
			THEN '30 or older'
		END [maternal_age_category]
	,bfd.tx_race_census [maternal_census_race]
	,bfd.tx_ethnicity_census [maternal_census_ethnicity]
INTO #t9
FROM #t8 AS t
LEFT JOIN rodis_wh.birth_fact bf ON t.id_birth = bf.cd_birth_fact
LEFT JOIN rodis.berd b ON t.id_birth = b.bc_uni
LEFT JOIN rodis_wh.birth_familial_dim bfd ON bf.birth_administration_id_birth_familial_maternal = bfd.id_birth_familial

-- Now we apply some logic to get an approximation of
-- the maternal race categories used by EPH.
SELECT *
	,CASE 
		WHEN has_birth_record = 'no'
			THEN NULL
		WHEN maternal_census_race IN ('White')
			THEN CASE 
					WHEN maternal_census_ethnicity IN ('Central or S. Am','Cuban','Mexican','Puerto Rican')
						THEN 'hispanic'
					ELSE 'white'
					END
		WHEN maternal_census_race IN ('Other Non-White','Unknown')
			THEN CASE 
					WHEN maternal_census_ethnicity IN ('Central or S. Am','Cuban','Mexican','Puerto Rican')
						THEN 'hispanic'
					ELSE 'other/unknown'
					END
		WHEN maternal_census_race = 'Black'
			THEN 'black'
		WHEN maternal_census_race = 'Hispanic'
			THEN 'hispanic'
		WHEN maternal_census_race IN ('Asian Indian','Chinese','Filipino','Guamanian','Hawaiian','Japanese','Korean','Other Asian','Samoan','Vietnamese')
			THEN 'asian/pacific islander'
		WHEN maternal_census_race = 'Native American'
			THEN 'native american'
		ELSE 'other/unknown'
		END [maternal_simple_race_ethnicity]
INTO #t10
FROM #t9
...
SELECT t.*
	...
	,CASE 
		...
		WHEN hlc.census_hispanic_latino_origin = 'Non-Hispanic, Black or African American Alone'
			THEN 'black'
		ELSE 'other/unknown'
		END [child_census_ethnicity]
INTO #t11
FROM #t10 AS t
LEFT JOIN dbo.PEOPLE_DIM AS pd ON t.id_child = pd.ID_PRSN
	AND pd.IS_CURRENT = 1
LEFT JOIN dbo.ref_lookup_hispanic_latino_census AS hlc ON pd.census_Hispanic_Latino_Origin_cd = hlc.census_hispanic_latino_origin_cd
...
SELECT *
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN child_census_race = 'White/Caucasian'
			THEN CASE 
					WHEN child_census_ethnicity IN ('other/unknown','white')
						THEN 'white'
					ELSE child_census_ethnicity
					END
		WHEN child_census_race IN ('Other Race','Unknown')
			THEN CASE 
					WHEN child_census_ethnicity IN ('hispanic','black','white')
						THEN child_census_ethnicity
					ELSE 'other/unknown'
					END
		WHEN child_census_race = 'Multiracial'
			THEN 'multiracial'
		WHEN child_census_race = 'Black/African American'
			THEN 'black'
		WHEN child_census_race IN ('Asian','Native Hawaiian/Other Pacific Islander')
			THEN 'asian/pacific islander'
		WHEN child_census_race = 'American Indian/Alaskan Native'
			THEN 'native american'
		ELSE 'other/unknown'
		END [child_simple_race_ethnicity]
INTO #t12
FROM #t11
...
SELECT t.*
	...
	CASE 
		...
		END [maternal_raw_education]
INTO #t13
FROM #t12 t
LEFT JOIN rodis_wh.birth_fact bf ON t.id_birth = bf.cd_birth_fact
LEFT JOIN rodis_wh.birth_familial_dim bfd ON bf.birth_administration_id_birth_familial_maternal = bfd.id_birth_familial
LEFT JOIN rodis_wh.location_dim ld ON bfd.id_city_current = ld.id_city
LEFT JOIN dbo.CALENDAR_DIM cd ON bf.id_calendar_dim_birth_child = cd.ID_CALENDAR_DIM
LEFT JOIN rodis_wh.education_dim ed ON bf.birth_administration_maternal_birth_familial_id_education = ed.id_education
LEFT JOIN rodis.ref_momedu rm ON ed.cd_education = rm.Code
LEFT JOIN rodis.ref_Momedu03 rm03 ON ed.cd_education = rm03.Code

-- As our last prep step, we create a simple version of maternal
-- education to match the EPH definition.
SELECT *
	,CASE
		WHEN maternal_raw_education IS NULL
			THEN NULL
		WHEN maternal_raw_education IN ('Unknown (09 or 99?)','Unknown/Not stated','education not recorded or invalid')
			THEN 'unknown'
		WHEN maternal_raw_education IN ('Associate degree','Bachelors degree','College graduate','Completed 1 year of college','Completed 2 years of college','Completed 3 years of college'
				,'Doctorate or Professional degree','Masters degree','Post Graduate Work (< 2003)','Some college, no degree')
			THEN 'some college or more'
		ELSE 'high school degree or less'
		END [maternal_simple_education]
INTO #t14
FROM #t13
...
INTO #t15
FROM #t14 t
LEFT JOIN dbo.INTAKE_FACT inf ON t.id_intake = inf.ID_INTAKE_FACT
LEFT JOIN base.rptIntakes_CA ric ON inf.ID_ACCESS_REPORT = ric.ID_ACCESS_REPORT
LEFT JOIN base.rptPlacement rp ON t.id_removal = rp.id_placement_fact
...
SELECT t.*
	...
	,CASE 
		...
		END [intake_reporter_mandate]
INTO #t16
FROM #t15 t
LEFT JOIN dbo.INTAKE_FACT inf ON t.id_intake = inf.ID_INTAKE_FACT
LEFT JOIN dbo.INTAKE_ATTRIBUTE_DIM iad ON inf.ID_ACCESS_REPORT = iad.ID_ACCESS_REPORT
LEFT JOIN ref.reporter_mandate_xwalk rmx ON iad.CD_RPTR_DSCR = rmx.cd_rptr_dscr
...
SELECT af.ID_PRSN_VICTIM [child_id]
	,af.ID_INTAKE_FACT [intake_id]
	,COUNT(*) [number_allegations]
	,SUM(IIF(atd.TX_ALLEGATION = 'Abandonment', 1, 0)) [abandonment]
	,SUM(IIF(atd.TX_ALLEGATION = 'Negligent Treatment or Maltreatment', 1, 0)) [neglect]
	,SUM(IIF(atd.TX_ALLEGATION = 'Physical Abuse', 1, 0)) [physical_abuse]
	,SUM(IIF(atd.TX_ALLEGATION = 'Sexual Abuse', 1, 0)) [sexual_abuse]
	,SUM(IIF(atd.TX_ALLEGATION = 'Sexual Exploitation', 1, 0)) [sexual_exploitation]
	,SUM(IIF(atd.TX_ALLEGATION = 'Death Caused by Neglect/Abuse', 1, 0)) [intake_death]
	,SUM(IIF(atd.TX_ALLEGATION = 'Emotional Abuse', 1, 0)) [emotional_abuse]
	,SUM(IIF(atd.TX_ALLEGATION = 'Exploitation', 1, 0)) [exploitation]
	,SUM(IIF(atd.TX_ALLEGATION = 'Medical Neglect', 1, 0)) [medical_neglect]
	,SUM(IIF(atd.TX_ALLEGATION = 'Mental Injury', 1, 0)) [mental_injury]
	,SUM(IIF(atd.TX_ALLEGATION = 'Prenatal Injury', 1, 0)) [prenatal_injury]
	,SUM(IIF(af.ID_INVESTIGATION_ASSESSMENT_FACT IS NOT NULL, 1, 0)) [investigations]
	,SUM(IIF(fd.CD_FINDING IN (11), 1, 0)) [duplicate_allegation]
	,SUM(IIF(fd.CD_FINDING IN (6), 1, 0)) [pending]
	,SUM(IIF(fd.CD_FINDING IN (1,2,3,4,9), 1, 0)) [no_finding]
	,SUM(IIF(fd.CD_FINDING IN (10), 1, 0)) [inconclusive]
	,SUM(IIF(fd.CD_FINDING IN (7,8), 1, 0)) [unfounded]
	,SUM(IIF(fd.CD_FINDING IN (5), 1, 0)) [founded]
INTO #alleg_invest_find
FROM dbo.ALLEGATION_FACT af
...
SELECT t.*
	...
	,CASE 
		...
		ELSE 'all no finding'
		END [overall_findings]
INTO #t17
FROM #t16 t
LEFT JOIN #alleg_invest_find aif ON t.id_child = aif.child_id
...
SELECT *
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN overall_findings IN (
				'no allegation record'
...
SELECT t.*
	,CASE 
		...
		WHEN t.id_intake IS NULL
			THEN 'unknown'
		END [ihs_provided]
	,ihs.first_ihs_date [ihs_start_date]
INTO #t19
FROM #t18 t
LEFT JOIN (
	...
	) ihs ON t.id_intake = ihs.id_intake_fact
...
SELECT *
	,CASE 
		WHEN ooh_provided = 'yes'
			AND ihs_provided = 'yes'
			AND ihs_start_date < ooh_start_date
			THEN 'ihs and ooh'
		WHEN ooh_provided = 'yes'
			AND ihs_provided = 'yes'
			AND ihs_start_date >= ooh_start_date
			THEN 'ooh only'
		WHEN ooh_provided = 'no'
			AND ihs_provided = 'yes'
			THEN 'ihs only'
		WHEN ooh_provided = 'yes'
			AND ihs_provided = 'no'
...
SELECT *
	,CASE 
		WHEN overall_services IN ('ihs and ooh','ihs only','ooh only','no ihs or ooh')
			THEN overall_services
		WHEN overall_services = 'ihs unknown - ooh provided'
			THEN 'ooh only'
		WHEN overall_services = 'ihs unknown - no ooh'
			THEN 'no ihs or ooh'
		END [simple_services]
INTO #t22
FROM #t21
...
FROM #t22 t
LEFT JOIN base.rptPlacement rp ON t.id_removal = rp.id_placement_fact
LEFT JOIN dbo.ref_lookup_cd_discharge_type_exits dte ON rp.cd_discharge_type = dte.cd_discharge_type
...
WHERE ca_record_source = 'inferred from removal'
	OR any_investigation = 'yes'
	OR (
		simple_findings NOT IN ('no allegation record','no investigation record','no allegations or findings')
		AND simple_findings IS NOT NULL
		)
	OR ihs_provided = 'yes'
	OR ooh_provided = 'yes'
...
WHERE ihs_provided = 'yes'
	OR ooh_provided = 'yes'
	AND simple_findings IN ('no allegation record','no investigation record','no allegations or findings')
...
SELECT *
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN child_birth_date > intake_date
			THEN 1
		ELSE 0
		END [intake_before_birth_fl]
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN intake_reporter_mandate = 'mandatory'
			THEN 1
		ELSE 0
		END [mandatory_reporter_fl]
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN screen_decision = 'screen in'
			THEN 1
		ELSE 0
		END [screen_in_fl]
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN alleged_phys_abuse = 'yes'
			THEN 1
		ELSE 0
		END [alleged_phys_abuse_fl]
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN alleged_sex_abuse = 'yes'
			THEN 1
		ELSE 0
		END [alleged_sex_abuse_fl]
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN alleged_neglect = 'yes'
			THEN 1
		ELSE 0
		END [alleged_neglect_fl]
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN any_allegation = 'yes'
			THEN 1
		ELSE 0
		END [any_allegation_fl]
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN screen_decision <> 'screen in'
			THEN NULL
		WHEN any_investigation = 'yes'
			THEN 1
		ELSE 0
		END [any_investigation_fl]
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN screen_decision <> 'screen in'
			OR any_investigation <> 'yes'
			THEN NULL
		WHEN simple_findings = 'substantiated'
			THEN 1
		ELSE 0
		END [any_substantiated_fl]
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN screen_decision <> 'screen in'
	...
	,CASE 
		WHEN has_ca_record = 'no'
			THEN NULL
		WHEN id_removal IS NULL
			THEN NULL
		WHEN removal_outcome = 'still in care'
			THEN 1
		ELSE 0
		END [still_in_care_fl]
	,CASE 
		WHEN has_ca_and_birth_record = 'yes'
			THEN 1
		ELSE 0
		END [has_ca_and_birth_record_fl]
INTO #t24
FROM #t23
	/*
TRANSITION
	...
	SELECT *
		,CASE 
			WHEN has_ca_record = 'no'
				OR intake_order = 1
				THEN NULL
			ELSE LAG(id_intake, 1) OVER (
					PARTITION BY id_birth
					,id_child ORDER BY intake_order
					)
			END [previous_id_intake]
		,CASE 
			WHEN has_ca_record = 'no'
				THEN NULL
			ELSE LEAD(id_intake, 1) OVER (
					PARTITION BY id_birth
					,id_child ORDER BY intake_order
					)
			END [next_id_intake]
		,CASE 
			WHEN has_ca_record = 'no'
				THEN NULL
			ELSE LEAD(days_previous_intake_to_intake, 1) OVER (
					PARTITION BY id_birth
					,id_child ORDER BY intake_order
					)
			END [days_intake_to_next_intake]
		,CASE 
			WHEN has_ca_record = 'no'
				OR intake_order = 1
				THEN NULL
			ELSE LAG(ihs_provided, 1) OVER (
					PARTITION BY id_birth
					,id_child ORDER BY intake_order
					)
			END [previous_ihs_provided]
		,CASE 
			WHEN has_ca_record = 'no'
				OR intake_order = 1
				THEN NULL
			ELSE LAG(ooh_provided, 1) OVER (
					PARTITION BY id_birth
					,id_child ORDER BY intake_order
					)
			END [previous_ooh_provided]
		,CASE 
			WHEN has_ca_record = 'no'
				THEN NULL
			ELSE LEAD(ihs_provided, 1) OVER (
					PARTITION BY id_birth
					,id_child ORDER BY intake_order
					)
			END [next_ihs_provided]
		,CASE 
			WHEN has_ca_record = 'no'
				THEN NULL
			ELSE LEAD(ooh_provided, 1) OVER (
					PARTITION BY id_birth
					,id_child ORDER BY intake_order
					)
			END [next_ooh_provided]
	FROM #t24
	)
,temp2
AS (
	SELECT *
		,CASE 
			WHEN days_previous_intake_to_intake <= 7
				AND (
					previous_ihs_provided = 'yes'
					OR previous_ooh_provided = 'yes'
					)
				AND ihs_provided IN ('no','unknown')
				AND ooh_provided = 'no'
				THEN 1
			WHEN days_previous_intake_to_intake <= 7
				AND previous_ihs_provided IN ('no','unknown')
				AND previous_ooh_provided = 'no'
				AND ihs_provided IN ('no','unknown')
				AND ooh_provided = 'no'
				THEN 1
			ELSE 0
			END [drop_for_previous]
		,CASE 
			WHEN days_intake_to_next_intake <= 7
				AND (
					next_ihs_provided = 'yes'
					OR next_ooh_provided = 'yes'
					)
				AND ihs_provided IN ('no','unknown')
				AND ooh_provided = 'no'
				THEN 1
...
INTO #t25
FROM #drops
WHERE drop_for_next != 1
	AND drop_for_previous != 1
	AND (
		simple_findings != 'duplicate intake'
		OR (
			has_birth_record = 'yes'
			AND has_ca_record = 'no'
			)
		)
	...
	SELECT *
		...
		IIF(ISNULL(id_intake, 0) != ISNULL(lag(id_intake, 1) OVER (
					...
					ORDER BY new_intake_order
						,new_intake_removal_order
					), 0), 1, 0) [new_intake]
		,
		-- Or a new removal.
		IIF(ISNULL(id_removal, 0) != ISNULL(lag(id_removal, 1) OVER (
					PARTITION BY id_birth
					,id_child ORDER BY new_removal_order
						,new_intake_removal_order
					), 0), 1, 0) [new_removal]
	FROM #t26
	)
...
AS (
	SELECT *
		...
		SUM(new_intake) OVER (
			PARTITION BY id_birth
			,id_child ORDER BY new_intake_removal_order
			) [new_cumulative_intakes]
		,
		-- And the same for removals.
		SUM(new_removal) OVER (
			PARTITION BY id_birth
			,id_child ORDER BY new_intake_removal_order
			) [new_cumulative_removals]
	FROM temp1
	)
,temp3
AS (
	SELECT *
		...
		ROW_NUMBER() OVER (
			PARTITION BY id_birth
			,id_child
			,new_cumulative_intakes ORDER BY new_intake_removal_order
			) [last_intake_lag]
		,
		-- And the same for removals. Note, however, that this
		-- isn't quite what we want for removals, because - unlike
		...
		ROW_NUMBER() OVER (
			PARTITION BY id_birth
			,id_child
			,new_cumulative_removals ORDER BY new_intake_removal_order
			) [bad_removal_lag]
	FROM temp2
	)
,temp4
...
SELECT *
	,LAG(intake_date, last_intake_lag) OVER (
		PARTITION BY id_birth
		,id_child ORDER BY new_intake_removal_order
		) [new_previous_intake_date]
	,LAG(removal_date, last_removal_lag) OVER (
		PARTITION BY id_birth
		,id_child ORDER BY new_intake_removal_order
		) [new_previous_removal_date]
INTO #t27
FROM temp4
...
SELECT *
	,FIRST_VALUE(intake_date) OVER (
		PARTITION BY id_birth
		,id_child ORDER BY new_intake_order
		) [new_first_intake_date]
	,FIRST_VALUE(removal_date) OVER (
		PARTITION BY id_birth
		,id_child
		-- We reuse some of our black magic sauce from above to insure
...
-- focus.
SELECT *
	,DATEDIFF(DAY, child_birth_date, new_first_intake_date) [new_days_birth_to_first_intake]
	,DATEDIFF(DAY, child_birth_date, new_first_removal_date) [new_days_birth_to_first_removal]
	,DATEDIFF(DAY, child_birth_date, intake_date) [new_days_birth_to_intake]
	,DATEDIFF(DAY, child_birth_date, removal_date) [new_days_birth_to_removal]
	,DATEDIFF(DAY, new_first_intake_date, intake_date) [new_days_first_intake_to_intake]
	,DATEDIFF(DAY, new_previous_intake_date, intake_date) [new_days_previous_intake_to_intake]
	,DATEDIFF(DAY, intake_date, removal_date) [new_days_intake_to_removal]
	,DATEDIFF(DAY, new_previous_removal_date, removal_date) [new_days_previous_removal_to_removal]
INTO #t29
FROM #t28
...
SELECT id_birth
	...
	,intake_date
	,removal_date
	,new_intake_order [intake_order]
	,new_removal_order [removal_order]
	,new_intake_removal_order [intake_removal_order]
	,new_cumulative_intakes [cumulative_intakes]
	,new_cumulative_removals [cumulative_removals]
	,new_first_intake_date [first_intake_date]
	,new_first_removal_date [first_removal_date]
	,new_previous_intake_date [previous_intake_date]
	,new_previous_removal_date [previous_removal_date]
	,new_days_birth_to_first_intake [days_birth_to_first_intake]
	,new_days_birth_to_first_removal [days_birth_to_first_removal]
	,new_days_birth_to_intake [days_birth_to_intake]
	,new_days_birth_to_removal [days_birth_to_removal]
	,new_days_first_intake_to_intake [days_first_intake_to_intake]
	,new_days_previous_intake_to_intake [days_previous_intake_to_intake]
	,new_days_intake_to_removal [days_intake_to_removal]
	,new_days_previous_removal_to_removal [days_previous_removal_to_removal]

CREATE procedure [dbo].[dispro_region_ssdi]
as
BEGIN
...
if object_id('tempdb..##dispro_region_ssdi') is not null drop table ##dispro_region_ssdi;
select
  year
	, intake_region
...
FROM ##dispro_region_ssdi du
LEFT JOIN 
	(select year, intake_region, measure, rate white_rate
	from ##dispro_region_ssdi

...
as
 set nocount on
    ...
    select @minmonthstart=min_date_any ,@maxmonthstart=max_date_any 
				,@mindate=min_date_any,@maxdate=max_date_any FROM ref_lookup_max_date where id=12;
			...
			INSERT INTO [prtl].[cache_pbcw3_params]
				...
				select 
					isnull((select max(qry_id) +1
						from prtl.[cache_pbcw3_params]),1)
					,@age_grouping_cd
					,@race_cd
					,@gender_cd
					,@init_cd_plcm_setng
...
if @bin_dep_cd!='0' 
				...
				set @mysql=CONCAT(@mysql,',year(prtl_pbcw3.start_date)
				,che.int_hash_key
			FROM prtl.ooh_point_in_time_measures prtl_pbcw3  ');
			if (@age_grouping_cd!='0')
					set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (select prm.*
										...
										on cast(arrValue as int)=age_grouping_cd
											) age on age.match_code=prtl_pbcw3.age_grouping_cd_mix');
				if (@race_cd!='0')
				set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (select eth.cd_race,eth.cd_origin,eth.match_code
									...
									on cast(arrValue as int)=eth.cd_race
											) eth on eth.match_code=prtl_pbcw3.cd_race and prtl_pbcw3.census_hispanic_latino_origin_cd = eth.cd_origin');
				if (@gender_cd!='0')
								set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (select pk_gndr,match_code
									from prm_gndr gdr
									join dbo.fn_ReturnStrTableFromList(',char(39),@gender_cd,char(39),',0) 
									on cast(arrValue as int)=gdr.pk_gndr) gdr on gdr.match_code=prtl_pbcw3.pk_gndr');
				if (@init_cd_plcm_setng!='0')
								set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (select fpl.init_cd_plcm_setng,fpl.match_code
										from prm_fpl  fpl
										join dbo.fn_ReturnStrTableFromList(',char(39),@init_cd_plcm_setng,char(39),',0)  sel 
										on cast(sel.arrValue as int)=fpl.init_cd_plcm_setng
										) fpl on fpl.match_code=prtl_pbcw3.init_cd_plcm_setng');
				if (@long_cd_plcm_setng!='0')
								set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (select lpl.long_cd_plcm_setng,lpl.match_code
										from prm_lpl    lpl
										join dbo.fn_ReturnStrTableFromList(',char(39),@long_cd_plcm_setng,char(39),',0)  sel 
										on cast(sel.arrValue as int)=lpl.long_cd_plcm_setng
										) lpl on lpl.match_code=prtl_pbcw3.long_cd_plcm_setng');
				if(@County_Cd!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (select  cnty.cd_cnty,cnty.match_code
										...
										) cnty on cnty.match_code=prtl_pbcw3.county_cd');
				if(@bin_los_cd!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
							...
							join [prm_los_max_bin_los_cd] los on los.bin_los_cd=cast(arrValue as int)
										) los on los.match_code=prtl_pbcw3.max_bin_los_cd');
				if(@bin_placement_cd!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (	select plc.bin_placement_cd,plc.match_code
										...
										) plc on plc.match_code=prtl_pbcw3.bin_placement_cd');
				if(@bin_ihs_svc_cd!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (	select ihs.bin_ihs_svc_cd,ihs.match_code
										...
										) ihs on ihs.match_code=prtl_pbcw3.bin_ihs_svc_cd');
				if(@cd_reporter_type!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (	select rpt.cd_reporter_type,rpt.match_code
										...
										) rpt on rpt.match_code=prtl_pbcw3.cd_reporter_type');
				if(@filter_access_type!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
							join ( 	select  acc.cd_access_type,acc.match_code
										...
										)  acc on acc.match_code=prtl_pbcw3.filter_access_type');
				if(@filter_allegation!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (select alg.cd_allegation,alg.match_code
										...
										) alg on alg.match_code=prtl_pbcw3.filter_allegation');
				if(@filter_finding!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (select fnd.cd_finding,fnd.match_code
										...
										) fnd on fnd.match_code=prtl_pbcw3.filter_finding');
				if(@filter_service_category!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
							join (	select srvc.cd_subctgry_poc_frc,srvc.match_code
										...
										) srv on srv.match_code=prtl_pbcw3.filter_service_category');
				if(@filter_service_budget!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
								join (	select cd_budget_poc_frc,match_code
										from prm_budg bud
											join dbo.fn_ReturnStrTableFromList(',char(39),@filter_service_budget,char(39),',0)  sel
												...
												) bud on bud.match_code=prtl_pbcw3.filter_service_budget');
				if(@bin_dep_cd!='0')
							set @mysql=CONCAT(@mysql,char(13),char(10),'
								join (select dep.bin_dep_cd,dep.match_code 
											...
											) dep on dep.match_code=prtl_pbcw3.bin_dep_cd');
				set @mysql=CONCAT(@mysql,char(13),char(10),'
				...
				group by prtl_pbcw3.qry_type
					...
					if OBJECT_ID(N'prtl.pbcw3_code',N'U') is not null
					begin
							truncate table prtl.pbcw3_code
							insert into prtl.pbcw3_code(mycode)
							select @mysql;
					end
					exec (@mysql)
		...
		if @fl_return_results=1					
            ...
            join ref_filter_dependency ref_dep on ref_dep.bin_dep_cd=pbcw3.bin_dep_cd
                   and pbcw3.cohort_entry_date >=ref_dep.min_filter_date
            join ref_filter_los ref_los on ref_los.bin_los_cd=pbcw3.bin_los_cd
			join [dbo].[ref_age_cdc_census_mix]  ref_age on ref_age.age_grouping_cd=pbcw3.age_grouping_cd
			join ref_lookup_gender ref_gdr on ref_gdr.[pk_gndr]=pbcw3.pk_gndr
			join ref_lookup_ethnicity_census ref_eth on ref_eth.[cd_race_census] = pbcw3.cd_race
			join [dbo].[ref_lookup_plcmnt]  ref_fpl on ref_fpl.[cd_plcm_setng] = pbcw3.init_cd_plcm_setng
			join [dbo].[ref_lookup_plcmnt]  ref_lpl on ref_lpl.[cd_plcm_setng] = pbcw3.long_cd_plcm_setng
			join [dbo].[ref_filter_nbr_placement] ref_plc on ref_plc.[bin_placement_cd]=pbcw3.[bin_placement_cd]
            ...
            join ref_filter_reporter_type ref_rpt on ref_rpt.cd_reporter_type=pbcw3.cd_reporter_type
			 join ref_filter_ihs_services ref_ihs on ref_ihs.[bin_ihs_svc_cd]=pbcw3.bin_ihs_svc_cd
            join ref_filter_access_type ref_acc on ref_acc.cd_access_type=pbcw3.cd_access_type
			join ref_filter_allegation ref_alg on ref_alg.cd_allegation=pbcw3.cd_allegation
			join ref_filter_finding ref_fnd on ref_fnd.cd_finding=pbcw3.cd_finding

	...
	CREATE procedure [prtl].[prod_build_cache_poc2ab_aggr](@permission_key datetime)
as
...
begin
	...
	if OBJECT_ID('tempDB..#x1x2') is not null drop table #x1x2
	create table #x1x2 (x1 float,x2 float)
	insert into #x1x2 select dbo.RandFn(),dbo.RandFn()
	...
	update statistics #find			
		...
		if OBJECT_ID('tempDB..#cachekeys') is not null drop table #cachekeys;
		   ...
		   select cast((int_param_key  * power(10.0,5)) as decimal(12,0))
			...
			declare @i int =0
			declare @stop int = (select count(distinct cd.QUARTER) -1 from CALENDAR_DIM  cd where cd.QUARTER>=(select  min_date_any from  ref_lookup_max_date where id=18)  and cd.quarter <=(select  max_date_any from  ref_lookup_max_date where id=18))
			...
			begin
				...
				insert into prtl.cache_poc2ab_aggr( 
						...
						SELECT    prtl_poc2ab.qry_type
								...
								, (select x1 from #x1x2)as x1  
								, (select x2 from #x1x2)  as x2

...
CREATE  view [prtl].[vw_referrals_grp] as 
	...
	select		distinct
		...
		left join (select grp.intake_grouper
					...
					left join (select distinct id_intake_fact from base.tbl_household_children where age_at_referral_dt<5
										) hh_under_12 on hh_under_12.id_intake_fact=intk.id_intake_fact
					left join (select distinct id_intake_fact from base.tbl_household_children where age_at_referral_dt<2

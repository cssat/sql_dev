...
CREATE procedure [prtl].[prod_build_cache_ooh_tables]( @permission_key datetime, @prm_procedure_id int)
as
	...
	begin
			...
			begin			
				...
				begin
					...
					set @mysql=@mysql + ',row_count=(select count(*) from prtl.'  + ((select xwlk.tbl_name from prtl.prtl_tables_last_update  tbl join prtl.tbl_procedure_xwalk xwlk on tbl.tbl_id=xwlk.tbl_id where xwlk.procedure_id=@procedure_id and tbl_seq_nbr=@loop ) )
 +  ')' + char(13) + char(10)
					set @mysql=@mysql + ',load_time_mins=dbo.fnc_datediff_mis(' + char(39) + convert(varchar,@starttime) + char(39) + ',' + char(39) + convert(varchar,@endtime) + char(39) + ')'
					set @mysql=@mysql +'where tbl_id='  +   ((select convert(varchar(2),xwlk.tbl_id)  from prtl.prtl_tables_last_update  tbl join prtl.tbl_procedure_xwalk xwlk on tbl.tbl_id=xwlk.tbl_id where procedure_id=@procedure_id and tbl_seq_nbr=@loop ))  + char(13
) + char(10)

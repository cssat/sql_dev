...
CREATE PROCEDURE [prtl].[sp_cache_pbcs2_insert_only](
...
as
 set nocount on
	...
	update statistics #fnd			
		...
		--	select @var_row_cnt_param,@var_row_cnt_cache
	if @var_row_cnt_param <> @var_row_cnt_cache
		 ...
		 --					join #cachekeys ck on ck.int_hash_key=pbcs2.int_hash_key
			...
			--			join (select distinct cd_allegation from #alg) alg on alg.cd_allegation=pbcs2.cd_allegation
			--			join (select distinct cd_finding from #fnd) fnd on fnd.cd_finding=pbcs2.cd_finding

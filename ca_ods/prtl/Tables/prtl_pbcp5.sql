CREATE TABLE [prtl].[prtl_pbcp5] (
    ...
    CONSTRAINT [PK_prtl_pbcp5] PRIMARY KEY CLUSTERED ([cohort_exit_year] ASC, [qry_type] ASC, [cd_discharge_type] ASC, [int_match_param_key] ASC, [bin_dep_cd] ASC, [max_bin_los_cd] ASC, [bin_placement_cd] ASC, [cd_reporter_type] ASC, [bin_ihs_svc_cd] ASC, [filter_access_type] ASC, [filter_allegation] ASC, [filter_finding] ASC, [mnth] ASC),
...
CREATE NONCLUSTERED INDEX [idx_year_params]
    ...
    INCLUDE([cd_discharge_type], [bin_dep_cd], [filter_finding], [mnth], [discharge_count], [max_bin_los_cd], [bin_placement_cd], [cd_reporter_type], [bin_ihs_svc_cd], [filter_access_type], [filter_allegation]);


GO
CREATE NONCLUSTERED INDEX [idx_fl_include_in_cohort_count]
    ON [prtl].[prtl_pbcp5]([cohort_count] ASC)
    INCLUDE([cohort_exit_year], [date_type], [qry_type], [int_match_param_key], [bin_dep_cd], [filter_finding], [discharge_count], [max_bin_los_cd], [bin_placement_cd], [cd_reporter_type], [bin_ihs_svc_cd], [filter_access_type], [filter_allegation]);

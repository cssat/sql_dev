		...
		--update row_num for last record for each int_researchID
		update tmp
		...
		begin
				...
				join ospi.Enrollment_2007 mtb
					...
					and mtb.row_num=1
						...
						--))
				left join ospi.enrollment_temp_jm ospi  -- don't want to update exit for primary key
				on mtb.int_researchID= ospi.int_researchID
				and mtb.districtID=ospi.districtID
				and mtb.SchoolCode=ospi.SchoolCode
				--and mtb.DateEnrolledInSchool=osp.DateEnrolledInSchool
				and mtb.DateExitedFromSchool=ospi.DateExitedFromSchool
				where osp.int_researchID between @loop and @stoploop
					and ospi.int_researchID is null
				commit tran t2;	
		...
		--into ##temp
		from ospi.Enrollment_2007 mt
		--exclude those we have updated where last enrollment school=first enrollment school
		...
		--exclude those matching primary key updated above
		left join ospi.enrollment_temp_jm t
